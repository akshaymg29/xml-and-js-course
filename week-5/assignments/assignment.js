...
const renderTable = () => {
    ...
    if (withFilters) {
      ...
      data = personnel.filter(({ first_name }) => first_name.toLowerCase().includes(nameTerm));
      if ( 0 == data.length)
      {
        data = personnel.filter(({ last_name }) => last_name.toLowerCase().includes(nameTerm));
        if ( 0 == data.length)
        {
            data = personnel.filter(({ email }) => email.toLowerCase().includes(nameTerm));
            if ( 0 == data.length)
            {
                data = personnel.filter(({ gender }) => gender.toLowerCase().includes(nameTerm));
                if ( 0 == data.length)
                    data = personnel.filter(({ ip_address }) => ip_address.toLowerCase().includes(nameTerm));
            }
        }
      }
    }
